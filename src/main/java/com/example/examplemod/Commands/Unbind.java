...
public class Unbind extends CommandBase {
    ...
    public void run(List<String> args) {
        ...
        ModuleBase tr = ExampleMod.MMN.getModuleByName(args.get(0));
        if (tr == null) {
            ClientHelper.SendClientNotif("Module doesnt exist kinda");
            return;
        }
        Map<KeyBindHelper, ModuleBase> newm = new HashMap<>();
        Map<KeyBindHelper, ModuleBase> rm = new HashMap<>();
        ExampleMod.bindings.forEach((keyBindHelper, moduleBase) -> {
